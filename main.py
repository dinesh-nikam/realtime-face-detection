    ...
    if accountno1 == '' or username == '':
        ...
        if cur.fetchone():
            ...
            def verify_otp():
                ...
                print(otp1)
                if otp1 == "":
                    messagebox.showerror("empty", "Enter the generated otp")
                else:
            ...
            def after_verification():
                ...
                def transfer():
                    messagebox.showinfo("Transfer", "Transfer option selected")

                def check_balance():
                    messagebox.showinfo("Check Balance", "Check Balance option selected")

                Button(options_frame, width=20, pady=7, text='Deposit', bg='#57a1f8', fg='white', border=2,
                       font=('Microsoft YaHei UI Light', 11, 'bold'), command=deposit).place(x=100, y=50)

                Button(options_frame, width=20, pady=7, text='Withdraw', bg='#57a1f8', fg='white', border=2,
                       font=('Microsoft YaHei UI Light', 11, 'bold'), command=withdraw).place(x=100, y=120)

                Button(options_frame, width=20, pady=7, text='Transfer', bg='#57a1f8', fg='white', border=2,
                       font=('Microsoft YaHei UI Light', 11, 'bold'), command=transfer).place(x=100, y=190)

                Button(options_frame, width=20, pady=7, text='Check Balance', bg='#57a1f8', fg='white', border=2,
                       font=('Microsoft YaHei UI Light', 11, 'bold'), command=check_balance).place(x=100, y=260)

                Button(options_frame, width=20, pady=7, text='Logout', bg='#57a1f8', fg='white', border=2,
                       font=('Microsoft YaHei UI Light', 11, 'bold'), command=root.destroy).place(x=100, y=330)
